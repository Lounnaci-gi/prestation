...
app.post('/api/devis', async (req, res) => {
  ...
  try {
    ...
    // Générer le code devis en utilisant la procédure stockée
    const codeDevisResult = await new Promise((resolve, reject) => {
      executeQuery('EXEC sp_GenererCodeDevis', [], (err, results) => {
        if (err) {
          console.error('Erreur lors de l\'exécution de la procédure stockée sp_GenererCodeDevis:', err);
          // En cas d'erreur, générer un code temporaire
          resolve(`DEV-${new Date(dateDevis).getFullYear()}-${Date.now().toString().slice(-6)}`);
        } else {
          resolve(results[0]?.CodeDevis || `DEV-${new Date(dateDevis).getFullYear()}-${Date.now().toString().slice(-6)}`);
        }
      });
    });
    
    const codeDevis = await codeDevisResult;
